...
class GROOTPolicyConfig(PolicyConfig):
    ...
    def build_policy(self, stats):
        return GROOTPolicy(self, stats)
    
    def build_policy_from_existing_model(self, model_path):
        return GROOTPolicy.load_from_safetensor(
            GROOTPolicy(self),
            model_path,
            self.device
        )
...
class GROOTPolicy(BasePolicy):
    ...
    def perform_inference(self) -> torch.Tensor:
        ...
        groot_obs = {
            ...
            "video": {},
            "language": {
                "annotation.human.task_description": [["Put the red cube in the box"]]
            }
        }
        for key in self.config.image_features.keys():
            groot_obs['video'][key] = np.array([[obs[key]]]) # Shape (Batch, ObsHorizon, H, W, C)
        for key in self.config.state_features.keys():
            groot_obs['state'][key] = np.array([[obs[key]]]) # Shape (Batch, ObsHorizon, D)
